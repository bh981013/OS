...
int sys_setpriority(void){
  ...
  if(argint(0, &n1)< 0 || argint(1, &n2)<0) return -1;
  setpriority(n1, n2);
  return 0;
}
    ...
    if (ismono==1){
      ...
      release(&ptable.lock);
        ...
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->level == 1 && p->state == RUNNABLE){
              if(temp==0)temp = p;
              if(temp->priority < p->priority) temp = p;
            }
          }  
        p = temp;
        if (p == 0) {
